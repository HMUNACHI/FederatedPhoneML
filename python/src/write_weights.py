...
from .quantization import map_layers_to_quantization_dtype, quantize_weights
...
def write_weights(
    ...
    for group_index, group in enumerate(weight_groups):
        ...
        quantization_dtype = map_layers_to_quantization_dtype(
        ...
        group = [
            (
                ...
                if e["name"] in quantization_dtype
                else e
            )
            for e in group
        ]
...
def _quantize_entry(entry, quantization_dtype):
    ...
    if data.dtype != "float32":
        return entry
    quantized_data, metadata = quantize_weights(data, quantization_dtype)
...
def _get_weights_manifest_for_group(group):
    ...
    for entry in group:
        ...
        dtype = (
            entry["quantization"]["original_dtype"]
            if is_quantized
            else entry["data"].dtype.name
        )
        var_manifest = {
            "name": entry["name"],
...
def _auto_convert_weight_entry(entry):
    ...
    if data.dtype in _AUTO_DTYPE_CONVERSION:
        ...
        print(
            ...
            + " and dtype "
            + data.dtype.name
            + " was auto converted to the type "
...
def _assert_valid_weight_entry(entry):
    ...
    if not isinstance(data, np.ndarray):
        raise ValueError(
            "Error dumping weight " + name + ", data " + "must be a numpy ndarray."
        )
...
def _assert_shard_size_bytes_valid(shard_size_bytes):
    if shard_size_bytes <= 0:
        raise ValueError(
            "shard_size_bytes must be greater than 0, but got %s" % shard_size_bytes
        )
    if not isinstance(shard_size_bytes, int):
        raise ValueError(
            "shard_size_bytes must be an integer, but got %s" % shard_size_bytes
        )
